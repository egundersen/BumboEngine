...
AttackPattern_Wall::AttackPattern_Wall(int width, int height, std::vector<std::vector<std::string>> &matrix_display, int &player_health, int number_of_attacks)
...
{
	createAttack( 0, width_, generateRandomNumber(0, height_ - 1), 1);
...
void AttackPattern_Wall::createAttack(int min_position_x, int max_position_x, int gap_height, int speed)
{
	Attack_Wall *attack;
	attack = new Attack_Wall(width_, height_, player_position_, matrix_, element_is_occupied_, min_position_x, max_position_x, gap_height, speed);
