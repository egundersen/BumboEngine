...
void Popup::displayPopup(int popup_position_x, int popup_position_y)
{
	for (int i = 0; i < popup_height_; i++)
		for (int j = 0; j < popup_width_; j++)
			screen_matrix_[i + popup_height_ / 2 + popup_position_y][j + popup_width_ + 1 + popup_position_x] = popup_matrix_[i][j];
}
...
void Popup::createPopupText()
{
	...
	addTextToMatrix(popup_width_ / 2, popup_height_ / 2 + 1, 'm', text_, popup_matrix_, popup_width_ - 3, popup_height_ - 2);
}
